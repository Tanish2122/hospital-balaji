...
async function getDoctor(id: string) {
  ...
  // Fallback: look up in local static data
  const local = localDoctors.find((d) => d.id === id);
  ...
  return {
    ...
    name: local.name,
    slug: local.id,
    qualification: local.qualifications.join(", "),
    experience_years: parseInt(local.experience.replace(/[^0-9]/g, "")) || 0,
    designation: local.specialty,
    bio: local.description,
    image_url: local.image,
    departments: { name: local.department },
...
export async function generateMetadata({
  ...
  return {
    title: `${doctor.name} | ${doctor.designation} | Balaji Hospital Jaipur`,
    description: doctor.bio ? doctor.bio.slice(0, 160) : `Consult with ${doctor.name} at Balaji Hospital Jaipur.`,
  };
}
...
const DoctorDetailPage = async ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  // Parse qualifications (assuming it might be stored as a comma-separated string or array)
  // Fallback to a default if not present since it's required by the UI
  let qualifications = [];
  if (doctorData.qualification) {
    try {
      qualifications = typeof doctorData.qualification === 'string' 
        ? doctorData.qualification.split(',').map(q => q.trim()) 
  ...
  const deptData = doctorData.departments as any;
  ...
  const doctor = {
    ...
    specialty: doctorData.designation || 'Specialist',
    image: doctorData.image_url || '/images/gallery/physiotherapy.png',
    experience: doctorData.experience_years ? `${doctorData.experience_years}+ Years` : 'Experienced',
    department: department || 'General',
    description: doctorData.bio || `Dr. ${doctorData.name} is a highly qualified specialist in the ${department || 'General'} department at Balaji Hospital.`,
    qualifications: qualifications.length > 0 ? qualifications : ["MBBS", "MD"],
    schedule: (doctorData as any).schedule ?? [
      { days: "Monday – Saturday", hours: "10:00 AM – 04:00 PM" },
      { days: "Sunday", hours: "On Call / Emergency Only" },
    ],
    services: (doctorData as any).services ?? [],
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-12">
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="p-8 bg-white rounded-3xl border border-slate-100 shadow-sm">
                ...
                <ul className="space-y-3">
                  {doctor.schedule.map((slot: { days: string; hours: string }, i: number) => (
                    <li
                      key={i}
            ...
            {doctor.services && doctor.services.length > 0 && (
              <section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {doctor.services.map((svc: string, i: number) => (
                     <div
                      key={i}
          ...
          <aside className="space-y-8">
            <div className="p-8 bg-slate-900 rounded-[2.5rem] text-white sticky top-32">
              <DoctorProfileCTA doctorName={doctor.name} />
